...
class DataPlan(BaseModel):
    thoughts: str = Field(default="Analyzing tabular dataset and executing data operations...")
    ...
    def normalize_action(cls, data: dict) -> dict:
        ...
        if "thoughts" not in data:
            data["thoughts"] = data.get("reasoning") or data.get("thought") or data.get("rationale") or "Analyzing tabular dataset..."
...
def build_data_agent(registry=None) -> StateGraph:
    ...
    def plan_node(state: dict):
        ...
        try:
            decision = safe_structured_invoke(llm, DataPlan, [sys_msg, human_msg])
            print(f"\n[DataAgent Thinking]:\n{decision.thoughts}\n")
            ...
            decision = DataPlan(thoughts=f"Recovered from parsing error: {e}", tool_name=fallback_tool, arguments={})
